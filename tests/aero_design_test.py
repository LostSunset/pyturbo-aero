...
class TestDesign(unittest.TestCase):
    ...
    def test_channel_2D(self):
        ...
        # Wavy Geometry
        t = np.linspace(0,1,1000)
        ss_ratio = 2*rotor_span*np.cos(5*math.pi*t)
        ps_ratio = 2*rotor_span*np.cos(3*math.pi*t)
        te_ratio = 0.5*rotor_span*np.cos(3*math.pi*t)
        le_ratio = 3*rotor_span*np.cos(12*math.pi*t)
        ...
        rotor3D_wavy.stretch_thickness_chord_te(SSRatio=ssratio_wave.get_wave(0,1),
                                                PSRatio=psratio_wave.get_wave(0.2,0.8,True),
                                                LERatio=leratio_wave.get_wave(0,1),
                                                TERatio=teratio_wave.get_wave(0,1),
                                                LE_wave_angle=LE_wave_angle,
                                                TE_wave_angle=TE_wave_angle,
                                                TE_smooth=0.90)
        ...
        channel.add_endwalls(zhub=hz, rhub=hr,
                             ...
                             rshroud_control=shroud_control_r,zshroud_control=shroud_control_z)
